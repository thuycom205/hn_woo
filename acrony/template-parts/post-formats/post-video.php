<div <?php post_class('post-single'); ?> >
    <?php 
        if(get_post_meta( $id, '_acrony_post_video_embed', true )){
            $video_url = get_post_meta( $id, '_acrony_post_video_embed', true );
        }else{
            $video_url = '';
        }
        if( !empty($video_url) && function_exists('acrony_video_embed_content') ):
            // Video Post Content Function
            echo acrony_video_embed_content(esc_url($video_url));
        else:   
