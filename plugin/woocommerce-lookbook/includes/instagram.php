...
if ( ! class_exists( 'VillaTheme_Instagram' ) ) {
	...
	class VillaTheme_Instagram {
		...
		public function __construct() {
			...
			$this->setting = new WOO_LOOKBOOK_Data();
		...
		public function import( $cache = true ) {
			...
			if ( ! $this->data || ! $cache ) {
				...
				if ( ! empty( $this->data ) && is_array( $this->data ) ) {
					set_transient( 'wlb_instagram_data', $this->data, $this->setting->get_ins_schedule() );
				} else {
					return false;
				}
			}
			...
			switch ( $this->setting->get_ins_image_status() ) {
				case 1:
					$post_status = 'publish';
					break;
				case 2:
			...
			foreach ( $this->data as $image ) {
				...
				if ( $this->setting->get_ins_duplicate() == 1 || ! $post_id ) {
		...
		public function get() {
			...
			$check_token = $this->check_token_live( $access_token );
			if ( $check_token !== false || $check_token !== null ) {
				...
				$limit  = $this->setting->get_ins_image_quantity();
//				$limit  = $this->setting->get_ins_image_quantity() > 12 ? 12 : $this->setting->get_ins_image_quantity();
