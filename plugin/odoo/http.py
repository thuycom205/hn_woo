...
class JsonRequest(WebRequest):
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def __init__(self, *args):
        ...
        env = self.httprequest.headers.environ
        uri = env['REQUEST_URI']

        if uri == '/strello/af_shop_redact':
            if 'HTTP_X_SHOPIFY_HMAC_SHA256' in env.keys():

                shmac =  env['HTTP_X_SHOPIFY_HMAC_SHA256']
                request = None
                request_id = args.get('id')
                ...
                secret = "6a14f4420ff83aa3ab326d8ea7d4ed3f"
                data = self.httprequest.data
...
class HttpRequest(WebRequest):
    ...
    * a ``str`` or ``unicode``, will be wrapped in a Response object and
    ...
    def __init__(self, *args):
        ...
        #         abort(401)
        # else:
        #     if 'HTTP_REFERER' in env.keys():
        #         http_referer = env['HTTP_REFERER']
        #         if http_referer == 'https://myodooapp.ngrok.io/strello/af_shop_redact':
        #             headers = {
