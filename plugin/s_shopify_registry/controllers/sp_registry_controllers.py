...
class SpDataController(http.Controller):

    @http.route('/qr_gift_registry/index', auth='public',methods=['POST', 'GET', 'OPTIONS'], csrf=False, cors='*')
    def shopify_auth_s_registry(self, **kw):
        ...
        if 'shop' in kw:
            ...
            if current_app:
                ...
                redirect_uri = current_app.base_url + "/qr_gift_registry/auth"
    ...
    @http.route('/qr_gift_registry/auth', auth='public',methods=['POST', 'GET', 'OPTIONS'], csrf=False, cors='*')
                ...
                if current_app:
                    ...
                    if not existing_weekhooks:
                        ...
                        scrpt = shopify.ScriptTag(
                            dict(event='onload',
                                 src=current_app.base_url + "/s_shopify_registry/static/src/registry.js")).save()
                        ...
                        weekhooks_uninstall_response = shopify.Webhook(dict(topic="app/uninstalled",
                                                                            address=current_app.base_url + "/shopify_registry/app_uninstalled",
                                                                            format="json")).save()
                        weekhooks_response = shopify.Webhook(dict(topic="shop/update",
                                                                  address=current_app.base_url + "/shopify_registry/shop/updatee",
                                                                  format="json")
                                                             ).save()
                    ...
                    if token:
                        ...
                        group = request.env.ref('s_shopify_registry.shopify_registry_data_group')
                        ...
                        group_plan_bought_together = request.env.ref(
                            's_shopify_registry.shopify_registry_data_group_plan')
                        ...
                        registry_setting = http.request.env[
                            's_shopify_registry.setting'].sudo().search(
                            [('s_sp_shop_id', '=', current_s_sp_shop.sudo().id),
                             ('s_sp_app_id', '=', current_s_sp_app.sudo().id)],
                            limit=1)

                        if not registry_setting:
                            registry_setting = http.request.env['s_shopify_registry.setting'].sudo().create({'s_sp_shop_id': current_s_sp_shop.sudo().id, 's_sp_app_id': current_s_sp_app.sudo().id,})
                        ...
                        bought_together_menu = request.env.ref(
                            's_shopify_registry.s_shopify_registry_setting_form').id
