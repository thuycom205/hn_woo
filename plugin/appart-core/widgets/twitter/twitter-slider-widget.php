...
class Appland_twitter_widget extends WP_Widget {
	/**
	 * @var array
	 */
	protected $defaults;
	...
	public function __construct() {
		parent::__construct(
			'zoom_twitter_widget',
			esc_html__( '(Theme) Twitter widget', 'appart-core' ),
			array(
				'classname'   => 'widget3 widget_twitter',
				'description' => esc_html__( 'Displays twits from your twitter account.', 'appart-core' ),
			)
		);

		$this->defaults = array(
			'title'                => esc_html__( 'Twitter Feed', 'appart-core' ),
			'tweet-limit'          => 5,
			'screen-name'          => '',
			'is_switch_style'      => false,
			'show-timestamp'       => true,
			'show-follow-button'   => false,
			'show-followers-count' => false
		);
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'appart-core' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>"/>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'screen-name' ); ?>"><?php esc_html_e( 'Twitter Username:', 'appart-core' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'tweet-limit' ); ?>"><?php esc_html_e( '# of Tweets Shown:', 'appart-core' ); ?></label>
		...
		<p>
			<input class="checkbox" type="checkbox" <?php checked( $instance['is_switch_style'] ); ?> id="<?php echo $this->get_field_id( 'is_switch_style' ); ?>" name="<?php echo $this->get_field_name( 'is_switch_style' ); ?>" />
			<label for="<?php echo $this->get_field_id( 'is_switch_style' ); ?>"><?php _e( 'Switch style', 'appart-core' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show-timestamp' ); ?>"><?php _e( 'Show Timestamp', 'appart-core' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show-follow-button' ); ?>"><?php _e(' Display Follow me button', 'appart-core' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show-followers-count' ); ?>"><?php _e(' Show follower count? ', 'appart-core'); ?></label>
		</p>
	...
	protected function display_tweets( $tweets, $instance ) {
        ...
        <div class="widget_inner row m0">
            ...
            foreach ( $tweets as $tweet ) :
                ...
                if($instance['is_switch_style']) {
                    ?>
                    <li>
                        <?php echo $text; ?>
                    <?php if ( $instance['show-timestamp'] ) : ?>
                        <span><i class="fa fa-twitter"></i> <?php echo $this->human_time_diff_maybe( $time ); ?> </span>
                    <?php endif; ?>
                    </li>
                    <?php
                }else {
                    ?>
                    <li>
                        <a href="<?php echo esc_url($link); ?>"><i class="fa fa-twitter"></i></a>
	...
	protected function display_follow( $instance ) {
		...
		<div class="zoom-twitter-widget__follow-me">
			<a class="twitter-follow-button" href="https://twitter.com/<?php echo esc_attr( $screen_name ); ?>" data-show-count="<?php echo ($show_followers_count ? 'true' : 'false'); ?>">
				<?php esc_html_e( 'Follow &commat;', 'appart-core' ); echo esc_html( $screen_name ); ?>
			</a>
		</div>
		<?php
	}
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			<p>
				...
				if ( 404 == $this->lastHttpCode ) {
					printf(
						esc_html__( 'Twitter Widget: Non-existent username.', 'appart-core' )
					);
				} else {
					printf(
						__( 'Twitter Widget misconfigured, check <a href="%1$s" target="_blank">settings page</a> and <a href="%2$s" target="_blank">read instructions</a>. Error code: %3$s.', 'appart-core' ),
	...
	protected function human_time_diff_maybe( $timestamp ) {
		if ( ( abs( time() - $timestamp ) ) < 86400 ) {
			return sprintf( __( '%1$s ago', 'appart-core' ), human_time_diff( $timestamp ) );
		} else {
