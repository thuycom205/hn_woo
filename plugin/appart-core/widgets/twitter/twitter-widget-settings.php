...
function zoom_twitter_widget_action_links( $links, $file ) {
	...
	$settings_link = sprintf(
		...
		esc_html__( 'Settings', 'positive-core' )
	);
...
function zoom_twitter_menu_page_create() {
	add_options_page(
		__( 'Twitter Widget by Financo', 'positive-core' ),
		__( 'Twitter Widget' ),
		'manage_options',
		'zoom_twitter_widget',
...
function zoom_twitter_widget_settings() {
	...
	<div class="wrap">
		<h2><?php __( 'Twitter Widget by Financo', 'positive-core' ); ?></h2>
		...
		<?php if ( $some_filled && 200 != $connection->lastHttpCode() ) : ?>
			...
			<div class="error">
				<p>
					<?php
					printf(
						__( 'Bad Authentification Data. Please <a href="%1$s" target="_blank">read instructions</a> and provide correct credentials.', 'positive-core' ),
		...
		<p>
			<?php _e( 'Display your most recent thoughts from twitter. First you need to setup API keys in order for widget to be able to display your tweets.', 'positive-core' ) ?>
		</p>

		<p>
			<?php
			printf(
				__( 'Navigate to documentation page and <a href="%1$s" target="_blank">read instructions</a>.', 'positive-core' ),
				'http://www.wpzoom.com/docs/twitter-widget-with-api-version-1-1-setup-instructions/'
			);
			?>
		</p>
